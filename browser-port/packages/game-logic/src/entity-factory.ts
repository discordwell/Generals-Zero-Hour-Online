...
export function createMapEntity(self: GL, 
  ...
  const entity: MapEntity = {
    ...
    debrisAnimationState: null,
    debrisAllowsModelColorChange: null,
    modelConditionInfos: renderAssetProfile.modelConditionInfos,
    transitionInfos: renderAssetProfile.transitionInfos,
    projectileBoneFeedbackEnabledSlotMask: renderAssetProfile.projectileBoneFeedbackEnabledSlotMask,
    x,
    y,
    z,
    rotationY,
    ...
    nominalHeight,
    selected: false,
    isSelectable: normalizedKindOf.has('SELECTABLE'),
    ...
    sourceObjectEnteredOrExitedFrame: 0,
    sourceObjectTriggerAreas: [],
    sourceObjectIPos: { x: 0, y: 0, z: 0 },
    sourceObjectLayer: 1,
    sourceObjectDestinationLayer: 0,
    sourceObjectSafeOcclusionFrame: 0,
    sourceObjectFormationId: 0,
    sourceObjectFormationOffset: null,
    obstacleFootprint,
    largestWeaponRange,
    totalWeaponAntiMask,
    ...
    sourceAIAttackMoveState: null,
    sourceAISimpleMoveState: null,
    sourceAITemporaryState: null,
    ...
    toppleOptions: 0,
    toppleStumpId: 0,
    // W3DTreeBuffer client-side topple state. C++ zeroes TTree memory and only
    // initializes m_toppleState on addTree; active topples fill the rest.
    w3dTreeBufferToppleState: 'UPRIGHT',
    w3dTreeBufferDeleted: false,
    w3dTreeBufferLocationX: 0,
    w3dTreeBufferLocationY: 0,
    w3dTreeBufferLocationZ: 0,
    w3dTreeBufferAngularVelocity: 0,
    w3dTreeBufferAngularAcceleration: 0,
    w3dTreeBufferToppleDirectionX: 0,
    w3dTreeBufferToppleDirectionY: 0,
    w3dTreeBufferToppleDirectionZ: 0,
    w3dTreeBufferAngularAccumulation: 0,
    w3dTreeBufferOptions: 0,
    w3dTreeBufferMatrix3D: new Array(12).fill(0),
    w3dTreeBufferSinkFramesLeft: 0,
...
export function extractContainProfile(self: GL, objectDef: ObjectDef | undefined): ContainProfile | null {
  ...
  const visitBlock = (block: IniBlock): void => {
    ...
    } else if (moduleType === 'PRISONBEHAVIOR' || moduleType === 'PROPAGANDACENTERBEHAVIOR') {
      // Source parity: PrisonBehavior and PropagandaCenterBehavior both extend OpenContain.
      // Prison-specific visual/brainwash runtime state is extracted separately.
      profile = {
        moduleType: 'OPEN',
        allowInsideKindOf,
        forbidInsideKindOf,
    ...
    if (blockType === 'BEHAVIOR' || blockType === 'UPDATE') {
      ...
      if (moduleType === 'STRUCTURECOLLAPSEUPDATE') {
        ...
        for (const tokens of oclEntries) {
          ...
          if (phaseIdx >= 0) {
            for (let i = 1; i < tokens.length; i++) {
              if (tokens[i]) phaseOCLs[phaseIdx]!.push(tokens[i]!);
            }
          }
        }
