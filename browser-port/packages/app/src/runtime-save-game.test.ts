...
function createSourceUpgradeModuleBlockData(upgradeExecuted: boolean): Uint8Array {
  const xferSave = new XferSave();
  xferSave.open('create-source-upgrade-module');
  try {
    xferSave.xferVersion(1);
    xferSave.xferVersion(1);
    xferSave.xferVersion(1);
...
function readSourceGameLogicObjectStates(data: ArrayBuffer) {
  const chunkData = readSaveChunkData(data, 'CHUNK_GameLogic');
  if (!chunkData) {
    return null;
  }
  const xferLoad = new XferLoad(chunkData.slice().buffer);
  xferLoad.open('read-source-game-logic-object-states');
  try {
    xferLoad.xferVersion(3);
    xferLoad.xferUnsignedInt(0);
    xferLoad.xferVersion(1);
    const tocCount = xferLoad.xferUnsignedInt(0);
    const tocById = new Map<number, string>();
    for (let index = 0; index < tocCount; index += 1) {
      const templateName = xferLoad.xferAsciiString('');
      const tocId = xferLoad.xferUnsignedShort(0);
      tocById.set(tocId, templateName);
    }
    const objectCount = xferLoad.xferUnsignedInt(0);
    const objects: Array<{ templateName: string | null; state: NonNullable<ReturnType<typeof parseSourceMapEntityChunk>> }> = [];
    for (let index = 0; index < objectCount; index += 1) {
      const tocId = xferLoad.xferUnsignedShort(0);
      const blockSize = xferLoad.beginBlock();
      const objectData = xferLoad.xferUser(new Uint8Array(blockSize)) as Uint8Array;
      xferLoad.endBlock();
      const parsed = parseSourceMapEntityChunk(objectData);
      if (parsed) {
        objects.push({
          templateName: tocById.get(tocId) ?? null,
          state: parsed,
        });
      }
    }
...
function readFirstSourceGameLogicObjectState(data: ArrayBuffer) {
  return readSourceGameLogicObjectStates(data)?.[0]?.state ?? null;
}
...
function parseSourceUpgradeModuleBlockData(data: Uint8Array) {
  const xferLoad = new XferLoad(data.slice().buffer);
...
function readTerrainVisualChunk(data: ArrayBuffer): {
  ...
  try {
    ...
    const trees: NonNullable<ReturnType<typeof readTerrainVisualChunk>>['trees'] = [];
    for (let index = 0; index < treeCount; index += 1) {
      const modelName = xferLoad.xferAsciiString('');
      const textureName = xferLoad.xferAsciiString('');
      const location = {
        x: xferLoad.xferReal(0),
        y: xferLoad.xferReal(0),
        z: xferLoad.xferReal(0),
      };
      const scale = xferLoad.xferReal(0);
      const sin = xferLoad.xferReal(0);
      const cos = xferLoad.xferReal(0);
      ...
      const toppleStateBytes = xferLoad.xferUser(new Uint8Array(4));
      const toppleState = new DataView(
        toppleStateBytes.buffer,
        toppleStateBytes.byteOffset,
        toppleStateBytes.byteLength,
      ).getInt32(0, true);
      const angularAccumulation = xferLoad.xferReal(0);
      const options = xferLoad.xferUnsignedInt(0);
      const matrixVersion = xferLoad.xferVersion(1);
      const matrix3D = Array.from({ length: 12 }, () => xferLoad.xferReal(0));
      const sinkFramesLeft = xferLoad.xferUnsignedInt(0);
      trees.push({
        modelName,
        textureName,
        location,
...
describe('runtime-save-game', () => {
  ...
  it('writes W3DTreeDraw live drawables into fresh CHUNK_TerrainVisual tree buffers', () => {
    const mapData = {
      heightmap: {
        width: 2,
        height: 2,
    ...
    const saveFile = buildRuntimeSaveFile({
      description: 'TerrainVisual Tree Save',
      mapPath: 'assets/maps/TestMap.json',
      mapData,
      cameraState: null,
    ...
    const terrainVisualChunk = readTerrainVisualChunk(saveFile.data);
    expect(terrainVisualChunk?.treeCount).toBe(1);
    const tree = terrainVisualChunk?.trees[0];
    expect(tree?.modelName).toBe('PTreeOak');
    expect(tree?.textureName).toBe('PTreeOak.tga');
    expect(tree?.location).toEqual({ x: 30, y: 40, z: 5 });
    expect(tree?.scale).toBe(1);
    expect(tree?.sin).toBeCloseTo(0.5, 5);
    expect(tree?.cos).toBeCloseTo(Math.cos(Math.PI / 6), 5);
    expect(tree?.drawableId).toBe(12);
    expect(tree?.angularVelocity).toBe(0);
    expect(tree?.angularAcceleration).toBe(0);
    expect(tree?.toppleDirection).toEqual({ x: 0, y: 0, z: 0 });
    expect(tree?.toppleState).toBe(0);
    expect(tree?.angularAccumulation).toBe(0);
    expect(tree?.options).toBe(0);
    expect(tree?.matrixVersion).toBe(1);
    expect(tree?.matrix3D).toEqual(new Array(12).fill(0));
    expect(tree?.sinkFramesLeft).toBe(0);
    expect(terrainVisualChunk?.propBufferVersion).toBe(1);
    expect(terrainVisualChunk?.trailingBytes).toBe(0);
  });
  ...
  it('appends generated source Object::xfer records for live objects missing source state', () => {
    const sourceGameLogicBytes = createSourceGameLogicChunkData();

    const saveFile = buildRuntimeSaveFile({
      description: 'Source GameLogic Generated Object',
      mapPath: 'assets/maps/SourceGeneratedObject.json',
      mapData: {
        heightmap: {
          width: 1,
          height: 1,
    ...
    expect(inspectGameLogicChunkLayout(readSaveChunkData(saveFile.data, 'CHUNK_GameLogic')!)).toMatchObject({
      layout: 'source_outer',
      objectTocCount: 2,
      objectCount: 2,
    });
    const objects = readSourceGameLogicObjectStates(saveFile.data);
    expect(objects?.map((object) => [object.templateName, object.state.objectId])).toEqual([
      ['RuntimeTank', 7],
      ['RuntimeDrone', 8],
    ]);
    const generated = objects?.find((object) => object.state.objectId === 8)?.state;
    expect(generated).toMatchObject({
      objectId: 8,
      teamId: 3,
      drawableId: 88,
      internalName: 'UNIT_SPAWNED',
      originalTeamName: 'TEAMUNIT',
      visionRange: 175,
      shroudClearingRange: 160,
      shroudRange: 120,
    ...
    expect(generated?.geometryInfo.boundingCircleRadius).toBeCloseTo(Math.hypot(4, 6));
    expect(generated?.geometryInfo.boundingSphereRadius).toBeCloseTo(Math.hypot(4, 6, 4));
  });

  it('fails loudly when a missing source Object::xfer record cannot be synthesized', () => {
    const sourceGameLogicBytes = createSourceGameLogicChunkData();

    expect(() => buildRuntimeSaveFile({
      description: 'Source GameLogic Unsynthesizable Object',
      mapPath: 'assets/maps/SourceUnsynthesizableObject.json',
      mapData: {
        heightmap: {
          width: 1,
          height: 1,
      ...
      + 'and cannot be synthesized: 8 <unknown-template>.',
    );
  });
  ...
  it('rewrites source UpgradeModule mux state from live executed module state', () => {
    const sourceGameLogicBytes = createSourceGameLogicChunkData(false, [{
      identifier: 'ModuleTag_StatusUpgrade',
      blockData: createSourceUpgradeModuleBlockData(false),
    }]);

    const saveFile = buildRuntimeSaveFile({
      description: 'source upgrade module rewrite',
      mapPath: 'Maps/RuntimeTank/RuntimeTank.map',
      mapData: {
        width: 1,
        height: 1,
      ...
      gameLogic: {
        ...
        captureSourceGameLogicRuntimeSaveState: () => ({
          ...
          spawnedEntities: [{
            ...
            z: 20,
            rotationY: 1.25,
            executedUpgradeModules: new Set(['upgrade-module-1']),
            upgradeModules: [{
              id: 'upgrade-module-1',
              moduleTag: 'MODULETAG_STATUSUPGRADE',
              moduleType: 'STATUSBITSUPGRADE',
            }],
          } as unknown as import('@generals/game-logic').MapEntity],
        }),
        resolveSourceObjectModuleTypeByTag: (templateName, moduleTag) =>
          templateName === 'RuntimeTank' && moduleTag === 'ModuleTag_StatusUpgrade'
            ? 'STATUSBITSUPGRADE'
            : null,
        captureBrowserRuntimeSaveState: () => ({ version: 1 }),
        getObjectIdCounter: () => 8,
    ...
    const firstObject = readFirstSourceGameLogicObjectState(saveFile.data);
    const module = firstObject?.modules.find((item) => item.identifier === 'ModuleTag_StatusUpgrade');

    expect(module).toBeDefined();
    expect(parseSourceUpgradeModuleBlockData(module!.blockData)).toEqual({
      upgradeExecuted: true,
    });
  });
  ...
  it('rewrites source SpectreGunshipUpdate modules from live runtime state', () => {
    ...
    expect(parseSourceSpectreGunshipUpdateBlockData(spectreModule!.blockData)).toEqual({
      nextCallFrameAndPhase: (43 << 2) | 2,
      initialTargetPosition: { x: 500, y: 600, z: 200 },
      overrideTargetDestination: { x: 510, y: 610, z: 210 },
      satellitePosition: { x: 520, y: 620, z: 220 },
      status: 1,
      orbitEscapeFrame: 175,
      gattlingTargetPosition: { x: 530, y: 630, z: 230 },
      positionToShootAt: { x: 540, y: 640, z: 240 },
      okToFireHowitzerCounter: 9,
      gattlingId: 88,
    });
  });
  ...
  it('rewrites source NeutronMissileUpdate modules from live runtime state', () => {
    ...
    expect(parseSourceNeutronMissileUpdateBlockData(missileModule!.blockData)).toEqual({
      nextCallFrameAndPhase: (43 << 2) | 2,
      state: 'ATTACK',
      targetPos: { x: 101, y: 303, z: 202 },
      intermedPos: { x: 111, y: 333, z: 222 },
      launcherId: 55,
      attachWeaponSlot: 7,
      attachSpecificBarrelToUse: 3,
      accel: { x: 1.5, y: 3.5, z: 2.5 },
      vel: { x: 4.5, y: 6.5, z: 5.5 },
      stateTimestamp: 99,
      isLaunched: true,
      isArmed: true,
      noTurnDistLeft: 44.25,
  ...
  it('rewrites source ParticleUplinkCannonUpdate modules from live runtime state', () => {
    ...
    expect(parseSourceParticleUplinkCannonUpdateBlockData(particleModule!.blockData)).toEqual({
      ...
      frames: 7,
      visualState,
      initialTargetPosition: { x: 44, y: 66, z: 3 },
      currentTargetPosition: { x: 55, y: 77, z: 6 },
      scorchMarksMade: 4,
      nextScorchMarkFrame: 144,
      nextLaunchFXFrame: 155,
      damagePulsesMade: 3,
