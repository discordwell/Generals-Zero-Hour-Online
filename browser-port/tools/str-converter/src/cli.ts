 ...
 * STR converter CLI.
 *
 * Converts .str (map mission string files) to JSON.
 *
 * Usage:
 *   str-converter --input <file|dir> --output <dir>
 */
...
function printUsage(): void {
  console.log(`Usage: str-converter --input <file|dir> --output <dir>

Options:
  --input   Path to a .str file or directory containing them
...
function discoverFiles(inputPath: string): string[] {
  ...
  if (stat.isFile()) {
    return inputPath.toLowerCase().endsWith('.str') ? [inputPath] : [];
  }
  if (stat.isDirectory()) {
    const files: string[] = [];
    ...
    for (const entry of entries) {
      ...
      if (!entry.name.toLowerCase().endsWith('.str')) continue;
...
function main(): void {
  ...
  if (files.length === 0) {
    console.log('No .str files found.');
    return;
  }

  console.log(`Found ${files.length} .str file(s) to convert.`);
  let success = 0;
  let failed = 0;
  let skipped = 0;
  ...
  for (const filePath of files) {
    ...
    const outputPath = path.join(args.output, rel.replace(/\.str$/i, '.str.json'));
  ...
  console.log(`\nDone. ${success} converted, ${skipped} skipped (empty), ${failed} failed.`);
