#!/usr/bin/env node
/**
 * WAK (Water Tracks) converter CLI.
 *
 * Converts .wak binary water track files to JSON.
 *
 * Usage:
 *   wak-converter --input <file|dir> --output <dir>
 */
...
function printUsage(): void {
  console.log(`Usage: wak-converter --input <file|dir> --output <dir>

Options:
  --input   Path to a .wak file or directory containing them
...
function discoverFiles(inputPath: string): string[] {
  ...
  if (stat.isFile()) {
    return inputPath.toLowerCase().endsWith('.wak') ? [inputPath] : [];
  }
  if (stat.isDirectory()) {
    const files: string[] = [];
    ...
    for (const entry of entries) {
      ...
      if (!entry.name.toLowerCase().endsWith('.wak')) continue;
...
function main(): void {
  ...
  if (files.length === 0) {
    console.log('No .wak files found.');
    return;
  }

  console.log(`Found ${files.length} .wak file(s) to convert.`);
  let success = 0;
  let failed = 0;
  ...
  for (const filePath of files) {
    ...
    const outputPath = path.join(args.output, rel.replace(/\.wak$/i, '.wak.json'));
    ...
    try {
      ...
      console.log(`  OK: ${rel} (${result.trackCount} tracks)`);
      success++;
    } catch (err) {
